...
{        
    ...
    double marker_size = 5.0; // 4.25; // cm
	...
	// int coder = CV_FOURCC('M', 'J', 'P', 'G');
    // inputVideo.set(cv::CAP_PROP_FOURCC, coder);    

    while (inputVideo.grab()) 
    {
        cv::Mat image;
		inputVideo.retrieve(image);//抓取视频中的一张照片
		flip(image,image,1);//1代表水平方向旋转180度
        Mat imageGray;        
        cvtColor(image,imageGray,CV_RGB2GRAY);   
        // threshold(imageGray, imageGray, 170, 255, CV_THRESH_BINARY);
        ...
        //求解旋转矩阵rvecs和平移矩阵tvecs
        myEstimatePoseSingleMarkers(corners, marker_size, cameraMatrix, distCoeffs, rvecs, tvecs);//求解旋转矩阵rvecs和平移矩阵tvecs
        for (int i = 0; i < ids.size(); i ++)
        {
            ...
            // drawFrameAxes(image, cameraMatrix, distCoeffs, rvecs[i], tvecs[i], 5, 3);//3.3.1 没有
            cout<<"  T :"<<tvecs[i]<<endl;
            cout<<"  R :"<<rvecs[i]<<endl;
        }
